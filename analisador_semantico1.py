...
class Visitor(NodeVisitor):
    ...
    def visit_ChuckOp(self, node):
        filhos = list(node.children())
        target = filhos[0][1]
        source = filhos[1][1]

        self.visit(source)
        expr_type = source.attrs['uchuck_type']

        self.visit(target)

        self.visit_VarDecl(target)
        var_type = target.attrs['uchuck_type']

        self._assert_semantic(var_type == expr_type, 2, node.coord, ltype=var_type, rtype=expr_type)
    ...
    def visit_Literal(self, node):
        # Primeiro tenta usar 'tipo' (se for seu AST)
        ...
        # Caso contrário, tenta deduzir tipo com base no valor
        elif hasattr(node, 'value'):
            valor = node.value
            if isinstance(valor, int):
                node.attrs['uchuck_type'] = IntType
            elif isinstance(valor, float):
                node.attrs['uchuck_type'] = FloatType
            elif isinstance(valor, str):
                node.attrs['uchuck_type'] = StringType
            else:
                self._assert_semantic(False, 1, node.coord, name=str(valor))
        else:
            # Se não tem nem 'tipo' nem 'value', erro genérico
            self._assert_semantic(False, 1, node.coord, name="literal")
    ...
    def visit_PrintStatement(self, node):
        ...
        self._assert_semantic(expr is not None, 1, node.coord, name="PrintStatement")
        
        self.visit(expr)
        expr_type = expr.attrs['uchuck_type']
    ...
    def visit_Type(self, node):
        typename = getattr(node, 'typename', None) or getattr(node, 'name', None)
        self._assert_semantic(typename is not None, 1, node.coord, name="type")
        self._assert_semantic(typename in self.typemap, 1, node.coord, name=typename)
        node.attrs['uchuck_type'] = self.typemap[typename]
    ...
    def visit_VarDecl(self, node):
        ...
        # Compatível com ASTs diferentes
        varname = getattr(node, 'identifier', None) or getattr(node, 'name', None)
        self._assert_semantic(varname is not None, 1, node.coord, name="VarDecl")

        self._assert_semantic(self.symtab.lookup(varname) is None, 9, node.coord, name=varname)

        # Tenta pegar node.type (esperado pelo professor) ou node.typename (string)
        typename_node = getattr(node, 'type', None)
        if typename_node is not None:
            # Já é nó Type
            self.visit(typename_node)
            dtype_type = typename_node.attrs['uchuck_type']
        else:
            # É string (typename), cria o nó Type
            typename_str = getattr(node, 'typename', None)
            self._assert_semantic(typename_str is not None, 1, node.coord, name="type")
            temp_node = Type(typename_str, coord=node.coord)
            self.visit(temp_node)
            dtype_type = temp_node.attrs['uchuck_type']
